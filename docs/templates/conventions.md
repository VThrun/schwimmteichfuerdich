...
- Use `BaseLayout.astro` for the shared page structure. Do not duplicate the
  header, footer, or metadata in individual pages.
- Treat all visible content as placeholder content during the design phase. In
  particular, do not add real prices, images, technical data, contact details,
  or legal text without explicit approval.
...
- Interactions must be keyboard-accessible and respect
  ...
  implementation.
- Store design decisions in `.codex/context/` and keep general working rules in
  `AGENTS.md`. Keep historical research separate from approved content.
